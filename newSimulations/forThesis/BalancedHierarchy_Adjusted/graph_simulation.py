"""
BalancedHierarchy_Adjusted - Adjusted for ~25,000 nodes comparison.

This scenario aims to achieve optimal load ordering (SM > Dir > CXO ≈ Mgr)
while maintaining comparable organization size to InterviewBased.
Target: Achieve ~25,000 nodes with balanced load distribution.
"""
...
CHILD_RANGES = {
    "CXO": (18, 23),         # ★ Increased for larger org size
    "Director": (12, 15),    # ★ High → Second highest load
    "SeniorManager": (8, 11), # ★ Medium → Highest load (sandwiched)
    "Manager": (6, 9),        # ★ Low → Minimal reporting load
    "Player": (0, 0),
}
