"""
Dir_Increased_SM_Reduced_Step3: Director管理スパン増加、SeniorManager管理スパン削減

パラメータ変更:
- Director→SM: [16, 21] (avg 18.5, +48%)
- SM→Mgr: [5, 7] (avg 6.0, -52%)
- Mgr→Player: [9, 12] (avg 10.5, +40%) ※ノード数維持のため調整
"""
...
CHILD_RANGES = {
    "CXO": (15, 20),
    "Director": (16, 21),      # 増加: [10,15] → [16,21]
    "SeniorManager": (5, 7),   # 削減: [10,15] → [5,7]
    "Manager": (9, 12),        # 調整: [5,10] → [9,12] ノード数維持
    "Player": (0, 0),
}
...
def main() -> None:
    ...
    lines = [
        "# Dir_Increased_SM_Reduced_Step3 Simulation Result",
        "",
