"""
Dir_Increased_SM_Reduced_Step2: Director管理スパン増加、SeniorManager管理スパン削減

パラメータ変更:
- Director→SM: [14, 19] (avg 16.5, +32%)
- SM→Mgr: [6, 9] (avg 7.5, -40%)
- Mgr→Player: [8, 11] (avg 9.5, +27%) ※ノード数維持のため調整
"""
...
CHILD_RANGES = {
    "CXO": (15, 20),
    "Director": (14, 19),      # 増加: [10,15] → [14,19]
    "SeniorManager": (6, 9),   # 削減: [10,15] → [6,9]
    "Manager": (8, 11),        # 調整: [5,10] → [8,11] ノード数維持
    "Player": (0, 0),
}
...
def main() -> None:
    ...
    lines = [
        "# Dir_Increased_SM_Reduced_Step2 Simulation Result",
        "",
