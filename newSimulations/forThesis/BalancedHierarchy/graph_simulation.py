"""
BalancedHierarchy Scenario - Achieving optimal load ordering.

This scenario aims to achieve the load order: SeniorManager > Director > CXO ≈ Manager
by carefully balancing child node counts across all hierarchical levels.

Target load distribution:
- SeniorManager: Highest (caught between upper and lower layers)
- Director: Second highest (many reports from SeniorManagers)
- CXO & Manager: Low and balanced (minimal reporting burden)
"""
...
CHILD_RANGES = {
    "CXO": (5, 8),           # ★ Low → Minimal reporting load
    "Director": (12, 15),    # ★ High → Second highest load
    "SeniorManager": (6, 9),  # ★ Medium → Highest load (sandwiched)
    "Manager": (3, 5),        # ★ Low → Minimal reporting load
    "Player": (0, 0),
}
