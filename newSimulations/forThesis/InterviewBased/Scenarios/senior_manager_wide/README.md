...
# Scenario: Senior Manager Wide Fan-out

このシナリオでは、SeniorManager → Manager の子ノード数レンジを **5〜20** に拡張し、
よりばらつきのある中間層構造で SimLoad の偏りを観察します。
その他のロジックやタスクルールは FinalStudy の標準設定と同一です。

- 実行方法  
  ```bash
  source .venv/bin/activate
  cd newSimulations/FinalStudy/Scenarios/senior_manager_wide
  python graph_simulation.py
  ```
  生成物（`RESULT.md`, `REPORT.md`, `SORTEDRESULT.md` など）はシナリオフォルダ直下に出力されます。
