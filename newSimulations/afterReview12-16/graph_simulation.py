...
def build_graph(seed: int | None = None) -> nx.DiGraph:
    ...
    while queue and graph.number_of_nodes() < MAX_NODES:
        ...
        remaining_capacity = MAX_NODES - graph.number_of_nodes()
        if remaining_capacity <= 0 or max_children == 0:
            continue

        desired = rng.randint(min_children, max_children)
        child_count = min(desired, remaining_capacity)
        if child_count < min_children and remaining_capacity >= min_children:
        ...
        for _ in range(child_count):
            ...
            queue.append((node_name, child_role))
